...
def login():
    ...
    if user["password"] == data["password"]:
        # Generate a session token or user ID for authentication
...
def reserve_book():
    ...
    reservation_data = {
        ...
        'email': _email,
        'phone': _phone,
        'reservation_end_date': days_reservation  
    }
...
@app.route('/book_details/<book_id>', methods=['GET'])
def book_details(book_id):
    _email = request.args.get('email')

    # Retrieve the book from the database using the provided book_id
    book = books.find_one({'_id': ObjectId(book_id)})
    if book is None:
        return Response("Book not found", status=404, mimetype="application/json")

    # Prepare the response dictionary with basic book details
    response = {
        'book_id': str(book['_id']),
        'title': book['title'],
        'author': book['author'],
    ...
    if user:
        if user.get('role') == 'admin':
            response['days_reservation'] = book['days_reservation']
            response['available'] = book['available']
            if not book['available']:
                reservation = reservations.find_one({'book_id': ObjectId(book_id)})
                if reservation:
                    user_details = reservation['user_details']
            ...
            if not book['available']:
                reservation = reservations.find_one({'book_id': ObjectId(book_id)})
                if reservation and reservation['user_details']['email'] == _email:
                    user_details = reservation['user_details']
                    response['reserved_by'] = {
                        'name': user_details['name'],
                        'surname': user_details['surname'],
        ...
        return Response("User or admin not found", status=404, mimetype="application/json")
